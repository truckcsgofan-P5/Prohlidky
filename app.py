...
dnes = datetime.now()
zacatek_aktualniho_mesice = dnes.replace(
...
pristi_mesic_datum = (zacatek_aktualniho_mesice + pd.Timedelta(days=32)).replace(
    day=1
)
...
st.page_link(
    "https://elektronicivm.streamlit.app/",
    label="✏️ Evidence závad",
    icon="🔗",
)
st.divider()
...
with tab_kbs:
    ...
    if sloupec_v1:
        ...
        if propadle_v1:
            st.error(f"🚨 **PROPADLÁ V1:** {', '.join(propadle_v1)}‼️")
        if tento_v1:
...
st.divider()
if st.button(
    "💾 Uložit všechny změny do Excelu na GitHub",
    type="primary",
    use_container_width=True,
):
